        ...
        try{
            ...
            let { phoneOrEmail, password } = req.body;
                ...
                if (!phoneOrEmail || !password) {
                    throw dependencies.exceptionHandling.throwError("Request Body must contain phoneOrEmail and password", 400);
                }
                ...
                const isPhone = /^[0-9]{10}$/.test(phoneOrEmail);
                    ...
                    if (!password) {
                        throw dependencies.exceptionHandling.throwError("Request Body must contain password", 400);
                    }
            
                    // const authresult = await authController.login({ phone, email, password });
                    const authresult = await authController.login({ phoneOrEmail, password, isEmail, isPhone });
